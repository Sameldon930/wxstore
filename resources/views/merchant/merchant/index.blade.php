            ...
            <table class="table table-bordered text-center table-hover table-responsive">
                ...
                @foreach($data as $item)
                    <tr>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->mobile }}</td>
                        <td>@if($item->isCheckedMerchant())商户已审核@else<span class="text-danger">商户未审核</span>@endif</td>
                        <td>{{ $item->created_at}}</td>
                        <td>
                            <a class="pretty-btn" href="{{route('merchant.merchant.show', ['merchant' => $item->id])}}">详情</a>
