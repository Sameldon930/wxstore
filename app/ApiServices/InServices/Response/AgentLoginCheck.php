 ...
 * api测试类
 */
class AgentLoginCheck extends BaseResponse implements InterfaceResponse
{
    /**
     * 接口名称 kui
     * @var string
     * 接口id 0005
     */
    protected $method = 'AgentLoginCheck';
    ...
    public function check_status(&$params)
    {
        ...
        if (Cache::store('file')->get('is_lock') == 2) {
            UserLiveness::updateLastLogined($params['id']);
            return true;
        }
    }
    ...
    //判断如果是商户的话不能登录代理端
    public function merchantlogin_verify(&$params)
    {
        $api_token = User::where('id',$params['id'])->first()->remember_token;
        $inspect = User::where('id',$params['id'])->first();
        if ($inspect->isMerchant()){
